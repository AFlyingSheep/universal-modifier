...
int main() {
  ...
  if (!ReadProcessMemory(h_process, (LPCVOID)pointer, &actual_address,
                         sizeof(DWORD_PTR), NULL)) {
    printf("Open memory error.\n");
    return 1;
  } else {
  ...
  //     if (!ReadProcessMemory(h_process, (LPCVOID)pointer, &pointer,
  //     sizeof(DWORD_PTR), NULL)) {
  //         printf("Open memory error.\n");
  //         return 1;
  //     }
  ...
  if (!ReadProcessMemory(h_process, (LPCVOID)KnightHealth::real_ptr,
                         &life_value, sizeof(int), NULL)) {
    printf("Open memory error.\n");
    return 1;
  }
  if (!ReadProcessMemory(h_process, (LPCVOID)KnightPower::real_ptr, &power,
                         sizeof(int), NULL)) {
    printf("Open memory error.\n");
    return 1;
  }
  ...
  while (true) {
    if (!WriteProcessMemory(h_process, (LPVOID)KnightPower::real_ptr,
                            &KnightPower::default_value, sizeof(int), NULL)) {
      printf("Write memory error.\n");
      return 1;
    }
    if (!WriteProcessMemory(h_process, (LPVOID)KnightHealth::real_ptr,
                            &KnightHealth::default_value, sizeof(int), NULL)) {
      printf("Write memory error.\n");
      return 1;
    }
